...
def replace_char_at!(str, char, idx)
  # loop through the string and if the index == i => replace the char there
  str.each_char.with_index do |char, i|
    if i == idx # if the current index is equal to idx where we want to change
      str[i] == char # make the character replacement here
    end
  end
end
