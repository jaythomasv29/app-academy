...
def ele_replace!(array, hash)
  array.each_with_index do |ele, i|
    if hash.has_key?(ele) # check to see if the array ele exists in the hash as a key
      array[i] = hash[ele] # modify the current element in the array to the hash value
    end
  end
end
